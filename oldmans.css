/* ================================================
   OLD MAN'S — Shoptet Disco — Custom CSS v3.9
...
/* Vyhledavani */
#om-header .om-search { flex: 1; max-width: 848px; margin: 0 16px; }
...
.footer-bottom {
  ...
  box-sizing: border-box !important;
  /* Šedé pozadí splývající se zbytkem patičky — bez tohohle měla lišta
     nativně bílé pozadí (a možná i border/box-shadow), takže vypadala
     jako bílý "rámeček" plovoucí na šedém pozadí kolem. */
  background: #f5f5f5 !important;
  border: none !important;
  box-shadow: none !important;
}
...
/* ══════════════════════════════════════════════
   STRÁNKA KOŠÍK (/kosik/)
   Skutečná struktura ověřená přes konzoli (20. 7. 2026) — Shoptet
   nativní košík. Vzhled podle klientova Tailwind mockupu (20. 7. 2026):
   2 sloupce (produkty 2/3 šířky, souhrn 1/3), zaoblené kartičky,
   "X" smazat v pravém horním rohu karty, minimalistický počítadlo
   kusů, slevový kód VŽDY viditelný (ne skrytý za "Mám slevový kupón"),
   souhrn s vlastním nadpisem/bezpečnostním textem/logy plateb (tyhle
   3 věci v nativním Shoptet DOM neexistují, přidává je JS).
   Vše scoped pod #cart-wrapper, ať se nedotkne ničeho jinde na webu. */

/* Kroky (Nákupní košík / Doprava & platba / Informace o vás) —
   VRÁCENO (21. 7. 2026, na žádost klienta: zákazník musí vidět, v jaké
   fázi objednávky je). Styl podle nativního Shoptet screenshotu:
   dokončené/aktivní kroky zelený kruh s číslem + spojovací čára,
   budoucí krok bílý kruh se šedým obrysem. Konkrétní strukturu tříd
   (.cart-header .step / .step.active / .step.done) řídí Shoptet
   nativně — my jen barvíme, na pozicování/rozvržení nesaháme, ať se
   nerozbije nativní krokový layout (viz lekce o nativních prvcích). */
...
/* Dvousloupcové rozvržení: PRODUKTY (celá tabulka) + pruh dopravy
   vlevo spolu s kupónem, SOUHRN objednávky vpravo — sticky, sjíždí
   s obsahem během scrollování (21. 7. 2026, na žádost klienta).
   POZOR: samotné CSS by nestačilo — .cart-inner a pruh dopravy jsou
   nativně SOUROZENCI .row.summary, ne uvnitř .col-md-8. JS
   (enhanceCartPage, krok 0) je fyzicky přesouvá dovnitř .col-md-8,
   teprve pak má tenhle flex co srovnat do dvou sloupců. */
...
#cart-wrapper .col-md-4[data-testid="tableCart"] {
...
/* Produkty + pruh dopravy teď žijí UVNITŘ .col-md-8 (viz JS) — jen
   dorovnat, ať vyplní celou jeho šířku (dřív měly vlastní max-width:55%
   z doby, kdy byly samostatný blok nad dvousloupcem; teď už je šířku
   řídí rodič .col-md-8 výš). */
#cart-wrapper .cart-inner[data-testid="tableCart"],
#cart-wrapper .box.box-bg-default {
  max-width: 100% !important;
  width: 100% !important;
  margin-right: 0 !important;
  box-sizing: border-box !important;
...
/* Pruh "Objednejte ještě za X Kč a budete mít dopravu ZDARMA" — ikona
   (nahraný SVG "free doprava.svg") + text v řádku, progress bar pod
   tím. Ikonu vkládá JS jako první dítě .extra.delivery.
   PŘEPSÁNO (21. 7. 2026, verze dodaná klientem): icon 38px s auto
   výškou (místo pevného 40×40 čtverce), align-items zpět na center,
   progress bar přes celou šířku vlastním řádkem pod textem, částka
   už NENÍ zeleně (JS už nepřidává třídu .om-delivery-amount — viz
   enhanceCartPage). POZN.: align-items:center byl původní stav, který
   dřív dělal dojem "dvou ikon" (viz starší komentáře v historii) — tahle
   verze to řeší jinak (užší 38px ikona + mezera 12px pod řádkem
   textu), takže sem tam vizuálně ověřit po nasazení. */
#cart-wrapper .box.box-bg-default {
  background: transparent !important;
  border: none !important;
  padding: 0 !important;
...
#cart-wrapper .om-cart-truck-icon {
  width: 38px !important;
  height: auto !important;
  flex-shrink: 0 !important;
  margin: 0 !important;
}
/* Všechny tučné texty v liště (částka i ZDARMA) černé — JS už
   nepřidává .om-delivery-amount, takže tohle jediné pravidlo stačí. */
...
#cart-wrapper .price-range > div:after {
  content: "" !important;
  position: absolute !important;
  right: -6px !important;
  top: 50% !important;
...
/* Tabulka produktů -> kartičky (tabulku "rozbalíme" na flex řádky,
   funkčnost formulářů/tlačítek uvnitř necháváme nativní, měníme jen
   vzhled/rozložení) */
...
#cart-wrapper .cart-table tbody { display: block !important; }
#cart-wrapper .cart-table-header { display: none !important; }
/* Rozvržení podle WP reference (20. 7. 2026): obrázek vlevo přes obě
   řady, název nahoře, POD názvem počítadlo kusů + "X Kč / ks" hned
   vedle, celková cena vpravo svisle na střed, "X" smazat v pravém
   horním rohu (absolutně, beze změny). Grid místo flexu — flex neumí
   "obrázek přes dva řádky obsahu" bez zásahů do DOM pořadí td. */
...
#cart-wrapper td.cart-p-image { grid-area: img !important; }
#cart-wrapper td.cart-p-image img {
...
#cart-wrapper td.p-name {
  grid-area: name !important;
  align-self: end !important;
  padding-right: 40px !important; /* rezerva na absolutní "X" v rohu */
}
#cart-wrapper td.p-name .main-link {
  font-weight: 600 !important;
  font-size: 14px !important;
  color: #1a1a1a !important;
  text-decoration: none !important;
}
#cart-wrapper td.p-name .main-link:hover { color: #9f1400 !important; }
/* Duplicitní mobilní verze dostupnosti + duplicitní tlačítko smazat
   uvnitř jména — Shoptet je renderuje 2× (jednou tady, jednou v
   p-total), necháváme viditelnou jen tu v p-total (viz níže).
   Dostupnost (Skladem X ks) v kartičce podle mockupu nezobrazujeme
   vůbec, ať je karta stejně minimalistická jako ve vzoru. */
#cart-wrapper td.p-name form.delete-cart-item,
#cart-wrapper td.p-name .product-table-name-footer,
#cart-wrapper .cart-table tr.removeable td.p-availability {
  display: none !important;
}
#cart-wrapper td.p-quantity {
  grid-area: qty !important;
  align-self: start !important;
  margin-top: 0 !important;
}
#cart-wrapper td.p-quantity .p-label { display: none !important; }
/* Minimalistický počítadlo kusů — jeden zaoblený obrys kolem celé
   skupiny (-, číslo, +), bez viditelných rámečků na jednotlivých
   tlačítkách, podle mockupu.
   OPRAVA (21. 7. 2026): +/- byly odsunuté do rohů (skoro mimo box),
   protože nativní tlačítka mají vlastní odsazení a znaky nebyly
   vycentrované. justify-content:center + flex-none na tlačítkách a
   flexbox centrování obsahu tlačítek to srovná. */
...
#cart-wrapper .quantity button.increase,
#cart-wrapper .quantity button.decrease {
...
#cart-wrapper .quantity button.increase:hover,
#cart-wrapper .quantity button.decrease:hover { background: #eee !important; }
/* Cena za kus — hned VEDLE počítadla (WP reference: "259 Kč / ks"
   normálním tmavým písmem, ne malé šedé vpravo) */
#cart-wrapper td.p-price {
  ...
  color: #444 !important;
  margin-top: 0 !important;
  line-height: 32px !important; /* svisle zarovnané na střed počítadla (výška 32px) */
}
#cart-wrapper td.p-price .p-label { display: none !important; }
#cart-wrapper td.p-price strong.price-final { color: #444 !important; font-weight: 500 !important; }
#cart-wrapper td.p-total {
...
#cart-wrapper td.p-total .p-label { display: none !important; }
#cart-wrapper td.p-total strong.price-final {
  font-size: 16px !important;
  font-weight: 800 !important;
  color: #1a1a1a !important;
}
/* Tlačítko smazat -> kulaté "X" v pravém horním rohu kartičky
   (absolutně pozicované, podle mockupu — ne v řádku s cenou) */
#cart-wrapper td.p-total form.inline {
  position: absolute !important;
  top: 14px !important;
  right: 14px !important;
}
#cart-wrapper td.p-total form.inline button.remove-item {
...
#cart-wrapper td.p-total form.inline button.remove-item:before,
#cart-wrapper td.p-total form.inline button.remove-item:after {
  content: "" !important;
  position: absolute !important;
  width: 12px !important;
...
#cart-wrapper td.p-total form.inline button.remove-item:before { transform: rotate(45deg) !important; }
#cart-wrapper td.p-total form.inline button.remove-item:after { transform: rotate(-45deg) !important; }
#cart-wrapper td.p-total form.inline button.remove-item:hover:before,
#cart-wrapper td.p-total form.inline button.remove-item:hover:after { background: #9f1400 !important; }

/* Slevový kód — VŽDY viditelný (podle mockupu), ne skrytý za
   "Mám slevový kupón". Tlačítko toggle schováváme a formulář natvrdo
   zobrazujeme; nativní JS toggle logika tím není nijak porušená, jen
   přebita vizuálně (kliknutí by beztak jen znovu zobrazilo to, co už
   je vidět). Oddělovací čárkovaná linka nad tím jako v mockupu.
   OPRAVA (21. 7. 2026): dřív byla linka na CELÉM .col-md-8 (fungovalo
   to, dokud .col-md-8 obsahoval jen kupón). Teď je uvnitř .col-md-8
   navíc pruh dopravy + tabulka produktů (viz JS přesun) — linka na
   celém sloupci by tak vyšla úplně nahoře nad vším, ne nad kupónem.
   Cíleno přímo na wrapper kupónu (.coupon-input.toggle-coupon-input). */
#cart-wrapper .coupon-input,
...
/* Přes celou šířku sloupce produktů (WP reference — input zabírá
   většinu řádku, černé tlačítko "Uplatnit" hned za ním) */
...
#cart-wrapper .discount-coupon input#discountCouponCode {
  flex: 1 !important;
  border: 1px solid #ddd !important;
  border-radius: 10px !important;
...
#cart-wrapper .discount-coupon button[type="submit"] {
...
#cart-wrapper .discount-coupon button[type="submit"]:hover { background: #333 !important; }

/* Souhrn (pravý sloupec) — bílá zaoblená karta se stínem, jako v
   mockupu (dřív bylo šedé pozadí přímo na celém sloupci — teď je
   šedé/bílé JEN uvnitř samotné karty se souhrnem cen). */
#cart-wrapper .col-md-4[data-testid="tableCart"] {
  background: transparent !important;
  padding: 0 !important;
}
/* Šedý zaoblený box BEZ rámečku/stínu (WP reference — dřív bílá karta
   s borderem a stínem) */
...
/* Nadpis "Souhrn objednávky" — vkládá JS, tady jen styl */
#cart-wrapper .om-cart-summary-title {
...
/* Řádky souhrnu (WP reference): "Celkem s DPH ......... 259 Kč" a
   "Doprava ......... od 149 Kč", label vlevo / hodnota vpravo na
   JEDNOM řádku, mezi řádky tenká linka. JS obalí nativní label +
   cenu do div.om-summary-line a přidá vlastní řádek Doprava. */
#cart-wrapper .om-summary-line {
  display: flex !important;
  justify-content: space-between !important;
...
#cart-wrapper .om-summary-line + .om-summary-line {
  border-top: 1px solid #e2e2e2 !important;
}
#cart-wrapper .price-label.price-primary {
  font-size: 15px !important;
  color: #1a1a1a !important;
  font-weight: 500 !important;
  display: inline-block !important;
}
#cart-wrapper strong.price.price-primary {
  display: inline-block !important;
  font-size: 17px !important;
  font-weight: 800 !important;
  color: #1a1a1a !important;
...
#cart-wrapper .om-summary-line .price-label {
  font-size: 15px !important;
  color: #1a1a1a !important;
  font-weight: 500 !important;
}
#cart-wrapper .om-summary-line strong {
  font-size: 17px !important;
  font-weight: 800 !important;
  color: #1a1a1a !important;
}
/* "Cena bez DPH" — v referenci není vůbec */
#cart-wrapper .price-label.price-secondary,
#cart-wrapper strong.price.price-secondary {
  display: none !important;
}
#cart-wrapper .next-step.next-step--cart {
  display: flex !important;
  flex-direction: column !important;
  gap: 10px !important;
}
/* ZELENÉ tlačítko "Pokračovat na dopravu a platbu ›" (WP reference —
   dřív červené "POKRAČOVAT"; text přepisuje JS, šipku přidává :after) */
#cart-wrapper .next-step-forward {
  background: #79b530 !important;
  color: #fff !important;
  border-radius: 12px !important;
...
#cart-wrapper .next-step-forward:after {
  content: "›" !important;
  margin-left: 10px !important;
  font-weight: 800 !important;
}
#cart-wrapper .next-step-forward:hover { background: #6aa028 !important; }
/* "Zpět do obchodu" — ve WP referenci není žádné druhé tlačítko */
#cart-wrapper .next-step-back {
  display: none !important;
}

/* Bezpečnostní text + loga plateb pod tlačítkem — oboje vkládá JS
   (v nativním Shoptet DOM na téhle stránce neexistují). */
#cart-wrapper .om-cart-security-text {
  text-align: center !important;
  font-size: 12px !important;
  color: #777 !important;
  font-weight: 500 !important;
...
#cart-wrapper .om-cart-payment-logos {
  display: block !important;
  max-width: 100% !important;
  height: auto !important;
  margin: 0 auto !important;
...
/* Mobil — jméno produktu na vlastním řádku, zbytek pod ním */
@media (max-width: 600px) {
  #cart-wrapper td.p-name { flex: 1 1 100% !important; }
}

/* Slidery "Nakupte produkty, které jsou ve slevě" / "Mohlo by vás
   zajímat" pod souhrnem objednávky — produkty natažené přes fetch
   z reálných kategorií (viz JS), karty stylované samostatně (NE
   závislé na body.type-category, protože tohle je stránka košíku). */
.om-cart-upsell {
  max-width: 1400px;
  margin: 40px auto 0;
  padding: 0 20px;
...
.om-cart-upsell-wrapper {
  position: relative;
}
/* Kartičky (.product) uvnitř zdědí vzhled z .products-block .product .p
   (stejné CSS jako Bestsellery/Omáčky ve slevě na homepage) — tady jen
   scroll mechanika navíc, protože nativní Shoptet "drag" slider JS na
   nově vložený obsah nenaběhne. */
.om-cart-upsell-track {
  display: flex !important;
  flex-wrap: nowrap !important;
  gap: 20px !important;
  overflow-x: auto !important; /* nutné pro programový scrollBy (smooth animace šipek) */
  scroll-behavior: smooth !important;
  -webkit-overflow-scrolling: touch !important;
  padding: 4px 4px 10px !important;
  margin: -4px -4px 0 !important;
  scrollbar-width: none !important;
  /* POZOR: žádný scroll-snap! Recyklační nekonečná smyčka (viz JS
     loopNext/loopPrev) přesouvá karty mezi konci a tiše koriguje
     scrollLeft — scroll-snap by se s tou korekcí pral a způsobil
     viditelné poskočení. */
}
.om-cart-upsell-track::-webkit-scrollbar { display: none; }
/* Šipky — stejný vizuální styl (žlutý kruh + chevron) jako "Vybrané
   recepty" na homepage, ale vlastní třída (ne .product-slider-navigation),
   ať nezdědíme homepage-specifický margin-top offset, který by tady
   seděl na jinou výšku karty. */
.om-cart-upsell-nav {
  position: absolute;
  top: 40%;
  transform: translateY(-50%);
...
.om-cart-upsell-nav:hover { background: #e6b800 !important; transform: translateY(-50%) scale(1.05); }
.om-cart-upsell-nav:before {
  content: "" !important;
  display: block !important;
...
.om-cart-upsell-nav-prev { left: -20px; }
.om-cart-upsell-nav-prev:before { transform: rotate(-135deg); margin-left: 3px; }
.om-cart-upsell-nav-next { right: -20px; }
.om-cart-upsell-nav-next:before { transform: rotate(45deg); margin-left: -3px; }
@media (max-width: 768px) {
  .om-cart-upsell-track .product { width: 78vw !important; min-width: 78vw !important; max-width: 78vw !important; }
  .om-cart-upsell-nav { width: 40px !important; height: 40px !important; }
  .om-cart-upsell-nav-prev { left: 4px; }
  .om-cart-upsell-nav-next { right: 4px; }
}

/* ── Kartičky v cart-upsell sliderech — STEJNÝ vzhled jako homepage
   Bestsellery/Omáčky ve slevě (20. 7. 2026). Doteď na košíku platila
   jen základní .products-block pravidla (rámeček, zelené tlačítko),
   ale celý detailní layout (název vlevo tučně, čtvercová fotka cover,
   cena + kompaktní tlačítko na jednom řádku) byl scoped POUZE na
   .product-slider-holder / .products-alternative / body.type-category
   — na košík nedopadl vůbec, proto karty vypadaly nativně/rozbitě.
   Duplikát bloku scoped na .om-cart-upsell-track (viz lekce 12:
   stejná struktura karet, jiný obalový kontejner = duplikovat pravidla). */
.om-cart-upsell-track .product .p {
  display: flex !important;
  flex-direction: column !important;
  height: 100% !important;
  border-radius: 16px !important;
...
.om-cart-upsell-track .product .p:hover {
  border-color: #ddd !important;
  box-shadow: 0 10px 30px rgba(0,0,0,.10) !important;
  transform: translateY(-4px) !important;
}
.om-cart-upsell-track .product .image,
.om-cart-upsell-track .product a.image {
  display: block !important;
  position: relative !important;
  width: 100% !important;
  aspect-ratio: 1 / 1 !important;
...
.om-cart-upsell-track .product .p-image {
  position: absolute !important;
  inset: 0 !important;
  padding: 0 !important;
  margin: 0 !important;
...
.om-cart-upsell-track .product .image img {
  position: absolute !important;
  inset: 0 !important;
  width: 100% !important;
  height: 100% !important;
...
.om-cart-upsell-track .product .p-in {
  display: flex !important;
  flex-direction: column !important;
  flex: 1 !important;
}
.om-cart-upsell-track .product .p-in-in {
  display: flex !important;
  flex-direction: column !important;
  flex: 1 !important;
  padding: 16px !important;
...
.om-cart-upsell-track .product .name {
  display: block !important;
  width: 100% !important;
  margin: 0 0 8px !important;
  padding: 0 !important;
...
.om-cart-upsell-track .product .name,
.om-cart-upsell-track .product .name span,
.om-cart-upsell-track .product .name a {
  font-size: 17px !important;
  font-weight: 700 !important;
  line-height: 1.3 !important;
  color: #1a1a1a !important;
...
.om-cart-upsell-track .product .ratings-wrapper {
  display: block !important;
  width: 100% !important;
  margin: 0 0 4px !important;
  padding: 0 !important;
...
.om-cart-upsell-track .product .stars-placeholder,
.om-cart-upsell-track .product .ratings-wrapper .stars,
.om-cart-upsell-track .product .ratings-wrapper [class*="star"],
.om-cart-upsell-track .product .ratings-wrapper .number-ratings,
.om-cart-upsell-track .product .ratings-wrapper .show-add-review {
  display: none !important;
  height: 0 !important;
  margin: 0 !important;
  padding: 0 !important;
}
.om-cart-upsell-track .product .availability {
  display: block !important;
  font-size: 15px !important;
  font-weight: 500 !important;
  line-height: 1.4 !important;
...
.om-cart-upsell-track .product .p-bottom {
  margin-top: auto !important;
  padding: 0 !important;
}
.om-cart-upsell-track .product .p-bottom > div {
  display: flex !important;
  align-items: center !important;
  gap: 10px !important;
  padding: 0 4px 0 0 !important;
...
.om-cart-upsell-track .product .p-bottom .prices {
  margin: 0 !important;
  text-align: left !important;
  flex-shrink: 0 !important;
  display: flex !important;
...
.om-cart-upsell-track .product .price-final {
  font-size: 17px !important;
  font-weight: 800 !important;
  color: #1a1a1a !important;
  white-space: nowrap !important;
...
.om-cart-upsell-track .product .price-before {
  display: block !important;
  font-size: 12px !important;
  font-weight: 500 !important;
  color: #aaa !important;
...
.om-cart-upsell-track .product .price-final.price--sale,
.om-cart-upsell-track .product ins .price {
  color: #9f1400 !important;
}
/* Tlačítko Do košíku — nativně schované třídou .no-buttons, odkrýt */
.om-cart-upsell-track .product .p-bottom.no-buttons .p-tools,
.om-cart-upsell-track .product .p-tools {
  display: inline-flex !important;
  margin: 0 0 0 auto !important;
  flex-shrink: 0 !important;
  flex-grow: 0 !important;
...
.om-cart-upsell-track .product .pr-action {
  margin: 0 !important;
  width: auto !important;
  display: inline-block !important;
}
.om-cart-upsell-track .product .add-to-cart-button,
.om-cart-upsell-track .product .to-cart-button,
.om-cart-upsell-track .product [class*="buy"] {
  display: inline-flex !important;
  align-items: center !important;
  justify-content: center !important;
  gap: 6px !important;
...
.om-cart-upsell-track .product .add-to-cart-button:hover,
.om-cart-upsell-track .product .to-cart-button:hover {
  background: #6aa028 !important;
  box-shadow: 0 4px 12px rgba(121,181,48,.30) !important;
  transform: translateY(-1px) !important;
}
.om-cart-upsell-track .product .p-desc,
.om-cart-upsell-track .product .p-code,
.om-cart-upsell-track .product .quantity {
  display: none !important;
}
